...
export class DocumentService {
  ...
  findByName(filename: string): Observable<Document> {
    return this.http.get<Document>(`${Config.webApiUrl}/v1/document/${filename}`);
  }

  getDocument(filename: string): Observable<Document> {
    return this.http.get<Document>(`${Config.webApiUrl}/v1/document/files/${filename}`);
  }
  ...
  public update(id: string, document: Document, file: File): Observable<Document> {
    const formData = new FormData();
    formData.append("file", file);
    formData.append("documentRequest", JSON.stringify(document));

    return this.http.put<Document>(`${Config.webApiUrl}/v1/document/${id}`, formData);
  }
  ...
  public updateByName(document: Document, file: File): Observable<Document> {
    ...
    return this.http.put<Document>(`${Config.webApiUrl}/v1/document/name`, formData);
  }
}
